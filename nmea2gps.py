...
def get_position(latitude,longitude,gps_secs,gps_nsecs):
...
if __name__ == '__main__':
    parser = argparse.ArgumentParser(description='Script that takes the raw gps data and cereates a file with timestamp x y z ')
    parser.add_argument('--output', help='out file')
    parser.add_argument('--input', help='raw gps data with nmea sentences')
    ...
    for line in f_in:	
        if "GGA" in line:
            ...
            timestamp, x, y ,z = get_position(longitude,latitude,seconds,nanoseconds)
